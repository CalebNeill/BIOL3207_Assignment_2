...
```{r}
#calculating standard error of each report
dfzr <- cbind(datafinal, zr = NA)

for(i in 1:nrow(datafinal))
  {
  dfzr$zr[i] <- escalc(data = datafinal, measure = "ROM", n1i = ctrl.n, n2i = oa.n, m1i = ctrl.mean, m2i = oa.mean, sd1i = ctrl.sd, sd2 = oa.sd, var.names = c("Zr", "V_Zr"))
}
...
```{r}
dftest <- datafinal[!(datafinal$ctrl.mean <=0 & datafinal$oa.mean > 0 | datafinal$ctrl.mean > 0 & datafinal$oa.mean <= 0),]
dftest
```

```{r}
escalc(measure = "ROM", n1i = ctrl.n, n2i = oa.n, m1i = ctrl.mean, m2i = oa.mean, sd1i = ctrl.sd, sd2 = oa.sd, data = dftest, var.names = c("Zr", "V_Zr"), append = TRUE)
